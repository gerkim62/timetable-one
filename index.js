...
if ('2serviceWorker' in navigator) {
...
const loader = document.querySelector('.ring-container')
...
updateLoadingMessage('Optimizing')
  const param = {
  ...
  domtoimage.toPng(node, param)
    .then(function(image) {
     // document.querySelector('.wrapper').style.transform = 'rotate(0deg)'
      updateLoadingMessage('Saving Timetable')
      
      window.saveAs(image, `Timetable-by-Gerison-on-${new Date().toString()}.png`);
      updateLoadingMessage('Done Saving')
      loading(false)
